from data.http_ohlc_fetcher import fetch_http_ohlc
import pandas as pd
...
def monthly_download():
    ...
    for datex in date_range:
        ...
        e_date = datex.date()
        current = fetch_http_ohlc(
            stock=stock, from_date=s_date, to_date=e_date, period=period
        )
        df = pd.DataFrame(
            current, columns=["date", "open", "high", "low", "close", "volume", "oi"]
        )
        df["date"] = pd.to_datetime(df["date"]).dt.tz_localize(None)
        # convert dattime format from 2021-03-01 09:15:00 to 2021-03-01 09:15:59
        df["date"] = df["date"].apply(lambda x: x.replace(second=59))
